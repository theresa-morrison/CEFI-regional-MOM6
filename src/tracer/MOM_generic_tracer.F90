...
#   include "version_variable.h"
  ...
  subroutine initialize_MOM_generic_tracer(restart, day, G, GV, US, h, tv, param_file, diag, OBC, &
    ...
    call g_tracer_initialize_sponges(G, GV, US, tv, CS, param_file, sponge_CSp, ALE_sponge_CSp, day)
  ...
  subroutine  g_tracer_initialize_sponges(G, GV, US, tv, CS, param_file, Layer_CSp, ALE_CSp, Time)
    ...
    do ! check for each generic tracer if it is nudged
      ...
      ! wish this could be "if this var is on this list instead.......
      call get_param(param_file, mdl, "DO_SPONGE_GT_"//trim(g_tracer_name), do_sponge_gt, &
                     "If true, then then generic tracer "//trim(g_tracer_name)//" is nudged.", &
                     default=.false.)
